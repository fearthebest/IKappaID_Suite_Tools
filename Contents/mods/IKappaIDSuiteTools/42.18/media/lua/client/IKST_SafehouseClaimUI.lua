...
function IKST_SafehouseClaimUI.close()
    if IKST_SafehouseClaimUI.instance then
        IKST_SafehouseClaimUI.instance:removeFromUIManager()
        IKST_SafehouseClaimUI.instance = nil
    end
end

local Panel = ISPanel:derive("IKST_SafehouseClaimUI")
...
function Panel:createChildren()
    ...
    self.actionBtns = {}
    local actions = IKST_SafehousePermissions.ACTIONS
    local cols = 3
    local btnW = math.floor((innerW - ((cols - 1) * 4)) / cols)
    local ax = pad
    for i, action in ipairs(actions) do
        local label = IKST.text("IGUI_IKST_SafehouseClaim_Action_" .. action, action)
        local btn = ISButton:new(ax, y, btnW, 22, label, self, Panel.onToggleAction)
        btn.internal = action
        btn:initialise()
        self:addChild(btn)
...
function Panel:loadPermsForScope()
    ...
    if self.scope == "user" then
        ...
        perms = userPerms or IKST_SafehousePermissions.emptyPerms()
    else
        perms = entry.groups[self.scope] or IKST_SafehousePermissions.emptyPerms()
    end
    self.draft = IKST_SafehousePermissions.copyPerms(perms)
...
function Panel:onToggleAction(button)
    if not self.draft then
        self.draft = IKST_SafehousePermissions.emptyPerms()
    end
...
function Panel:onSave()
    ...
    IKST.dispatchCommand(self.player, IKST.CMD.safehouseClaimSetPerms, {
        x = self.refX, y = self.refY, w = self.refW, h = self.refH,
        scope = scope,
        username = username,
        perms = self.draft,
    })
...
function Panel:new(player, x, y, w, h)
    local pw, ph = 380, 300
    ...
    o.refW = w
    o.refH = h
    o.scope = IKST_SafehousePermissions.GROUP_EVERYONE
    o.draft = IKST_SafehousePermissions.emptyPerms()
    IKST_Chrome.applyPanelColors(o)
    return o
end

function IKST_SafehouseClaimUI.open(player, x, y, w, h)
    IKST_SafehouseClaimUI.close()
    local panel = Panel:new(player, x, y, w, h)
    panel:initialise()
    ...
    IKST_SafehouseClaimUI.instance = panel
end
