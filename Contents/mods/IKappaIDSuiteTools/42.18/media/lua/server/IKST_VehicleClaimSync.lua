...
function IKST_VehicleClaimSync.buildClaimsMap()
    ...
    for k, entry in pairs(data.byId) do
        if type(entry) == "table" then
            claims[k] = IKST_VehicleClaimMirror.copyEntry(entry)
            count = count + 1
        end
    end
    return claims, count
...
function IKST_VehicleClaimSync.bootstrapPayload()
    local claims, count = IKST_VehicleClaimSync.buildClaimsMap()
    return {
...
function IKST_VehicleClaimSync.sendBootstrap(player)
    ...
    IKST.deliverClientCommand(player, IKST.CMD.vehicleClaimBootstrap, IKST_VehicleClaimSync.bootstrapPayload())
end
...
function IKST_VehicleClaimSync.broadcastBootstrap()
    ...
    local payload = IKST_VehicleClaimSync.bootstrapPayload()
    if sendServerCommand then
        sendServerCommand(IKST.MODULE, IKST.CMD.vehicleClaimBootstrap, payload)
    end
end
...
function IKST_VehicleClaimSync.broadcastPatch(payload)
    ...
    if sendServerCommand then
        sendServerCommand(IKST.MODULE, IKST.CMD.vehicleClaimPatch, payload)
    end
end
...
function IKST_VehicleClaimSync.afterMutate(op, vehicleId, entry)
    ...
    local _, count = IKST_VehicleClaimSync.buildClaimsMap()
    local payload = {
        rev = rev,
        op = op,
    ...
    if op == "set" and entry then
        payload.entry = IKST_VehicleClaimMirror.copyEntry(entry)
    end
