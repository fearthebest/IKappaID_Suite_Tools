...
function IKST_Enforcement.grabBlocked(self)
    ...
    if type(worldItem.getItem) == "function" then
        item = worldItem:getItem()
    end
    if not item then
        return false
    end
    ...
    if type(item.getContainer) == "function" then
        src = item:getContainer()
    end
    local dest = nil
    if type(self.character.getInventory) == "function" then
        dest = self.character:getInventory()
    end
...
function IKST_Enforcement.wrapTransfer()
    ...
    if type(ISInventoryTransferAction.isValid) == "function"
        and not IKST_Enforcement.alreadyWrapped(ISInventoryTransferAction, "transfer_isValid") then
    ...
    if type(ISInventoryTransferAction.perform) == "function"
        and not IKST_Enforcement.alreadyWrapped(ISInventoryTransferAction, "transfer_perform") then
...
function IKST_Enforcement.wrapGrab()
    if not ISGrabItemAction or type(ISGrabItemAction.isValid) ~= "function" then
        return
    end
...
function IKST_Enforcement.wrapVehicleIsValid(classTable, actionKey, wrapKey)
    if not classTable or type(classTable.isValid) ~= "function" then
        return
    end
