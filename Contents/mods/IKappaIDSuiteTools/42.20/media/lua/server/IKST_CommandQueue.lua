-- Server JVM: tick-budgeted world ops (loot zone, vehicle prune, tiles bulk).
-- MP: only the server mutates; clients receive batchProgress / result mirrors.
...
function IKST_CommandQueue.dropJob(job)
    if not job then
        return
    end
    job.results = nil
...
function IKST_CommandQueue.enqueue(player, label, items, handler, onComplete)
    ...
    local q = IKST_CommandQueue.queues[key]
    while #q.pending >= IKST_CommandQueue.MAX_PENDING do
        local dropped = table.remove(q.pending)
        IKST_CommandQueue.dropJob(dropped)
    end
...
-- Core entry: same as enqueue; used by loot/vehicles/tiles.
function IKST.enqueueWorldOp(player, label, items, handler, onComplete)
    if not IKST_CommandQueue then
        require "IKST_CommandQueue"
    end
    if not IKST_CommandQueue or type(IKST_CommandQueue.enqueue) ~= "function" then
        return false
    end
    IKST_CommandQueue.enqueue(player, label, items, handler, onComplete)
    return true
end
