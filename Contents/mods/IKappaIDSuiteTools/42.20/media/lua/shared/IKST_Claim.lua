...
function IKST_Claim.walkDrawRect(x1, y1, z1, x2, y2, z2)
    ...
    if w < IKST_Claim.MIN_DIM or h < IKST_Claim.MIN_DIM then
        return false, "zone too small", x, y, w, h
    end
    if w > IKST_Claim.MAX_DIM or h > IKST_Claim.MAX_DIM then
        return false, "zone too large", x, y, w, h
    end
    if z1 ~= nil and z2 ~= nil and math.floor(tonumber(z1) or 0) ~= math.floor(tonumber(z2) or 0) then
        return false, "same floor only", x, y, w, h
    end
    return true, nil, x, y, w, h
end
