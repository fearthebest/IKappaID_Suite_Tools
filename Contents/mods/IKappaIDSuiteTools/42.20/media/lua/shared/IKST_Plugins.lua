...
function IKST.isAddonActive(modId)
    ...
    if getActivatedMods then
        ...
        if mods then
            if type(mods.contains) == "function" and mods:contains(modId) then
                return true
            end
            if type(mods.indexOf) == "function" and mods:indexOf(modId) >= 0 then
                return true
            end
...
function IKST.Plugins.canUseAdmin(pluginId, player)
    local spec = registry[pluginId]
    if not spec then
        return false
    end
    if not IKST.Plugins.isActive(pluginId) then
    ...
    if type(spec.canUseAdmin) == "function" then
        return spec.canUseAdmin(player) == true
    end
    return IKST_Access and IKST_Access.canUseStaffTools and IKST_Access.canUseStaffTools(player) == true
end
