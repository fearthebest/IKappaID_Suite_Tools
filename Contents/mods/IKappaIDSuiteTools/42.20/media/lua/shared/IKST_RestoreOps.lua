...
function IKST_RestoreOps.capturePlayer(player)
    ...
    local xp = type(player.getXp) == "function" and player:getXp() or nil
    IKST_RestoreOps.eachPerk(function(perk)
        ...
        if perkId and not IKST_RestoreOps.PARENT_PERKS[perkId] then
            local level = type(player.getPerkLevel) == "function" and player:getPerkLevel(perk) or 0
            local currentXP = 0
...
function IKST_RestoreOps.isJournalItem(item)
    return item and type(item.getFullType) == "function" and item:getFullType() == IKST.JOURNAL_TYPE
end
