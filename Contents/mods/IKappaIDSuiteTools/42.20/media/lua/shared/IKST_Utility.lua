...
function IKST.setUtilityOnServer(which, on)
    ...
    -- Server JVM only (dedicated or listen-host server side of command handler).
    if IKST.runsOnServerJvm and not IKST.runsOnServerJvm() then
        return false
    end
...
-- SP only: apply sandbox utility modifiers locally. MP always uses gated quickWater/quickPower.
