...
function IKST_VehicleKeys.keyInContainer(container, keyId)
    ...
    for i = 0, keys:size() - 1 do
        ...
        if item and type(item.getKeyId) == "function" and item:getKeyId() == keyId then
            return true
        end
    end
    return false
...
function IKST_VehicleKeys.playerHasVehicleKey(player, vehicle)
    ...
    if type(vehicle.isKeysInIgnition) == "function" and vehicle:isKeysInIgnition() then
        return true
    end
    local keyId = type(vehicle.getKeyId) == "function" and vehicle:getKeyId() or -1
    if keyId == nil or keyId < 0 then
        return true
    end
    local inv = type(player.getInventory) == "function" and player:getInventory()
