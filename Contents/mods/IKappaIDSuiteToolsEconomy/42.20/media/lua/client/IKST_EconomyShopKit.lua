...
function IKST_EconomyShopKit.isKitItem(item)
    return item and type(item.getFullType) == "function" and item:getFullType() == IKST_Economy.SHOP_KIT_TYPE
end
...
function IKST_EconomyShopKit.armPlacePick(player, item)
    player = IKST.resolvePlayer(player)
    ...
    IKST_WorldPick.armCommand(player, IKST.VIEW.economy, IKST.CMD.economyShopPlace, {
        itemId = item:getID(),
    }, { notifyKey = "IGUI_IKST_Economy_ClickPlaceShop" })
    return true
end
...
function IKST_EconomyShopKit.onInventoryMenu(playerNum, context, items)
    ...
    local opt = context:addOption(IKST.text("IGUI_IKST_Economy_PlaceShopKit", "Place shop at click"), player, function()
        IKST_EconomyShopKit.armPlacePick(player, item)
    end)
