...
function IKST_EconomyPanel:selectedValuableRow()
    ...
    local entry = list.items and list.items[list.selected]
...
function IKST_EconomyUI.onSnapshot(args)
    ...
    IKST_EconomyUI._snap.autoAccept = args.autoAccept ~= false
    IKST_EconomyUI._snap.payReq = args.payReq
    local hist = args.history
    if type(hist) ~= "table" then
        hist = {}
    end
    IKST_EconomyUI._snap.history = hist
...
-- Top of the shop row list; grows when owner/balance/status text pushes content down.
...
function IKST_EconomyUI.onServerResult(args)
    ...
    local displayMsg = args.message and IKST.formatServerResult(args.message, args) or nil
    if IKST_EconomyUI.Window then
        if displayMsg then
            IKST_EconomyUI.Window:setStatus(displayMsg)
        end
        IKST_EconomyUI.Window:refreshBalances()
    end
    ...
    if shopCmd and IKST_EconomyUI.VendWindow then
        if displayMsg then
            IKST_EconomyUI.VendWindow:setStatus(displayMsg, args.success == true)
        end
