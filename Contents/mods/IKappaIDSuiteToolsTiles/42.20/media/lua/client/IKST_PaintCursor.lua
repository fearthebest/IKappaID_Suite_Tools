...
function IKST_PaintCursorManager.arm(player, mode)
    ...
    if not IKST_Access.canUseTools(player) then
        IKST.notify(player, IKST.text("IGUI_IKST_AdminRequired", "Admin access required"), false)
        return
    end
    if not IKST.ensurePaintCursor() then
        IKST.notify(player, IKST.text("IGUI_IKST_PaintToolLoadFailed", "Paint tool failed to load"), false)
        return
    end
...
function IKST_PaintCursorManager.syncSprite(player)
    ...
    if mode ~= IKST.PAINTER_MODES.paint and mode ~= IKST.PAINTER_MODES.replace
        and mode ~= IKST.PAINTER_MODES.wall then
        return
    end
