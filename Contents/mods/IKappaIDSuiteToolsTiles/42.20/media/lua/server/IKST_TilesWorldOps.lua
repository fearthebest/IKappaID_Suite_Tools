...
function IKST_TilesWorldOps.locationMutationBlocked(player, x, y, z)
    if not IKST_Policy then
        require "IKST_Policy"
    end
    local allowed, reason = IKST_Policy.locationAllowedAtCoord(player, x, y, z, "tiles")
...
function IKST_TilesWorldOps.claimEditBlocked(player, x, y, z)
    local blocked = select(1, IKST_TilesWorldOps.locationMutationBlocked(player, x, y, z))
    return blocked == true
end
...
function IKST_TilesWorldOps.syncSquare(square)
    ...
    if type(square.RecalcAllWithNeighbours) == "function" then
        square:RecalcAllWithNeighbours(true)
    elseif type(square.RecalcProperties) == "function" then
        square:RecalcProperties()
    end
    if IKST.isMultiplayerSession and IKST.isMultiplayerSession() and type(square.transmitModdata) == "function" then
...
function IKST_TilesWorldOps.removeMultiTileObject(obj)
    ...
    for i = 0, list:size() - 1 do
        ...
        local partSq = part and type(part.getSquare) == "function" and part:getSquare()
        if partSq then
...
function IKST_TilesWorldOps.placeTree(square, spriteName, player)
    if not square or not spriteName or spriteName == "" then
        return false
    end
    if player then
        local blocked = select(1, IKST_TilesWorldOps.locationMutationBlocked(player, square:getX(), square:getY(), square:getZ()))
        if blocked then
            return false
        end
    end
...
function IKST_TilesWorldOps.placeBush(square, spriteName, player)
    if not square or not spriteName or spriteName == "" then
        return false
    end
    if player then
    ...
    return IKST_TilesWorldOps.placeSprite(square, spriteName, player)
end

function IKST_TilesWorldOps.placeRestored(square, item, player)
    local record = IKST_TilesWorldOps.normalizeRestoreRecord(item)
    if not record then
        return false
    end
    if record.kind == "tree" then
        return IKST_TilesWorldOps.placeTree(square, record.sprite, player)
    end
    if record.kind == "bush" then
        return IKST_TilesWorldOps.placeBush(square, record.sprite, player)
    end
    if record.kind == "floor" then
        return IKST_TilesWorldOps.replaceFloorSprite(square, record.sprite, player)
    end
    return IKST_TilesWorldOps.placeSprite(square, record.sprite, player)
end
...
function IKST_TilesWorldOps.placeSprite(square, spriteName, player)
    if not square or not spriteName or spriteName == "" then
        return false
    end
...
function IKST_TilesWorldOps.replaceFloorSprite(square, spriteName, player)
    if not square or not spriteName or spriteName == "" then
        return false
    end
    if player then
        ...
        if sq and entry.sprites then
            for i = #entry.sprites, 1, -1 do
                ...
                if record then
                    ...
                    if allow and IKST_TilesWorldOps.placeRestored(sq, record, player) then
                        restored = restored + 1
                    end
                end
            end
...
function IKST_TilesWorldOps.removeObjectFromSquare(square, obj, isTile)
    ...
    if safeTileRemove and IKST_TilesWorldOps.isMultiTileObject(obj) then
        local objSquare = type(obj.getSquare) == "function" and obj:getSquare()
...
function IKST_TilesWorldOps.containerItemsForObject(obj)
    local out = {}
    ...
    local counts = {}
    local order = {}
    local function addOne(it)
        if not it then
            return
        end
        local name = nil
        if type(it.getDisplayName) == "function" then
            name = it:getDisplayName()
        end
        if (not name or name == "") and type(it.getName) == "function" then
            name = it:getName()
        end
        if (not name or name == "") and type(it.getType) == "function" then
            name = it:getType()
        end
        ...
        local n = 1
        if type(it.getCount) == "function" then
            n = tonumber(it:getCount()) or 1
        end
        if n < 1 then
            n = 1
        end
    ...
    for i, name in ipairs(order) do
        ...
        out[#out + 1] = { name = name, count = counts[name] }
    end
    return out
end

function IKST_TilesWorldOps.inspectSquare(square, auditContainers)
    local items = {}
    if not square then
        return items
    end
    ...
    if objects then
        for i = 0, objects:size() - 1 do
            ...
            if auditContainers then
                local contents = IKST_TilesWorldOps.containerItemsForObject(obj)
                if #contents > 0 then
                    row.containerItems = contents
                end
            end
            items[#items + 1] = row
        end
...
function IKST_TilesWorldOps.runCleanup(mode, x, y, z, player, rewindLabel, objectIndex)
    local blocked, reason = IKST_TilesWorldOps.locationMutationBlocked(player, x, y, z)
    if blocked then
...
function IKST_TilesWorldOps.runBatch(player, squares, mode, label)
    ...
    IKST_CommandQueue.enqueue(player, label, coords, function(c)
        local blocked, reason = IKST_TilesWorldOps.locationMutationBlocked(player, c.x, c.y, c.z)
        if blocked then
            return false, reason or "square protected"
        end
