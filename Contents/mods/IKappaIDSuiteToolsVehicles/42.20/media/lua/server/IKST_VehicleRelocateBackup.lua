...
function IKST_VehicleRelocateBackup.finishRestore(backupId, newVehicle, oldClaimId)
    ...
    if IKST_VehicleClaim and oldClaimId then
        if IKST_VehicleIdentity and IKST_VehicleIdentity.isDurableKey(oldClaimId)
            and type(IKST_VehicleClaim.remapAfterRelocate) == "function" then
            IKST_VehicleClaim.remapAfterRelocate(oldClaimId, newVehicle, target)
        elseif IKST_VehicleClaim.remapVehicleId then
...
function IKST_VehicleRelocateBackup.restoreAtCoords(vehicleId, x, y, z, angle, playerObj)
    ...
    local oldClaimId = origin and origin.claimKey or (origin and origin.vehicleId) or vehicleId
