...
-- Forward-declare: captureItem and captureItemsList call each other;
-- captureItem also calls copyPrimitiveModData (Lua locals are not hoisted).
local captureItem
local captureItemsList
local copyPrimitiveModData
local applyItemRecord
local restoreContainerItems

copyPrimitiveModData = function(source, dest)
...
captureItemsList = function(items)
    if not items then
        return {}
    end
    local out = {}
...
captureItem = function(item)
    ...
    if type(item.getModData) == "function" then
        copyPrimitiveModData(item:getModData(), rec.modData)
    end
    local keyId = call0(item, "getKeyId", nil)
    if keyId ~= nil and keyId >= 0 then
        rec.keyId = keyId
    end
    if type(item.getInventory) == "function" then
        local sub = item:getInventory()
        if sub then
            local nested = captureItemsList(call0(sub, "getItems", nil))
...
local function captureContainer(part)
    ...
    return captureItemsList(call0(container, "getItems", nil))
end
...
local function readColor(vehicle)
    ...
    return {
        ...
        s = type(vehicle.getColorSaturation) == "function" and vehicle:getColorSaturation() or nil,
        v = type(vehicle.getColorValue) == "function" and vehicle:getColorValue() or nil,
    }
end
...
function VS.capture(vehicle)
    ...
    local snap = {
        ...
        keyId = call0(vehicle, "getKeyId", nil),
        parts = {},
        modData = {},
    }
    ...
    for i = 0, partCount - 1 do
        local part = type(vehicle.getPartByIndex) == "function" and vehicle:getPartByIndex(i) or nil
        if part then
            local partId = call0(part, "getId", nil)
            if partId and partId ~= "" then
                local content = nil
                if type(part.isContainer) == "function" and part:isContainer() and type(part.getContainerContentAmount) == "function" then
...
applyItemRecord = function(item, itemRec)
    if not item or type(itemRec) ~= "table" then
        return
    end
    ...
    if type(itemRec.modData) == "table" and type(item.getModData) == "function" then
        copyPrimitiveModData(itemRec.modData, item:getModData())
    end
    if itemRec.keyId ~= nil and itemRec.keyId >= 0 then
        if type(item.setKeyId) == "function" then
            item:setKeyId(itemRec.keyId)
        elseif type(item.syncKeyId) == "function" then
            item:syncKeyId(itemRec.keyId)
        end
    end
    if type(itemRec.items) == "table" and type(item.getInventory) == "function" then
        local sub = item:getInventory()
        if sub then
            restoreContainerItems(sub, itemRec.items)
        end
    end
end

restoreContainerItems = function(container, items)
...
local function restoreVehicleKeyId(vehicle, snap)
    if not vehicle or type(snap) ~= "table" then
        return false
    end
    local keyId = snap.keyId
    if keyId == nil or keyId < 0 then
        return true
    end
    if type(vehicle.setKeyId) ~= "function" then
        return false
    end
    vehicle:setKeyId(keyId)
    if type(vehicle.getKeyId) == "function" and vehicle:getKeyId() == keyId then
        if vehicle.transmitKeyId then
            vehicle:transmitKeyId()
        end
        return true
    end
    return false
...
function VS.formatResultMessage(code)
    ...
    if IKST_Policy and IKST_Policy.formatDenyMessage then
        ...
        if policyMsg and policyMsg ~= "" and policyMsg ~= msg then
            return policyMsg
        end
    end
    if msg == "relocated" then
        return IKST.text("IGUI_IKST_Vehicle_Relocated", "Vehicle relocated")
    end
    if msg == "relocated_keys_invalid" then
        return IKST.text("IGUI_IKST_Vehicle_RelocatedKeysInvalid",
            "Vehicle relocated — physical keys may no longer work")
    end
    return msg
end
...
function VS.apply(vehicle, snap)
    ...
    if type(snap.parts) == "table" then
        for _, rec in ipairs(snap.parts) do
            local part = type(vehicle.getPartById) == "function" and vehicle:getPartById(rec.id) or nil
            if part then
                ...
                if rec.item and rec.item.fullType then
                    ...
                    if newItem and part.setInventoryItem then
                        applyItemRecord(newItem, rec.item)
                        if type(part.getInventoryItem) == "function" and part:getInventoryItem() then
                ...
                elseif type(part.getInventoryItem) == "function" and type(part.setInventoryItem) == "function" and part:getInventoryItem() then
                    part:setInventoryItem(nil)
                ...
                if rec.content ~= nil and part.setContainerContentAmount then
                    ...
                    local wheelIndex = type(part.getWheelIndex) == "function" and part:getWheelIndex() or -1
    ...
    if type(vehicle.getModData) == "function" and type(snap.modData) == "table" then
        copyPrimitiveModData(snap.modData, vehicle:getModData())
    end
    local keysInvalid = false
    if snap.keyId ~= nil and snap.keyId >= 0 then
        if not restoreVehicleKeyId(vehicle, snap) then
            keysInvalid = true
            if type(vehicle.getModData) == "function" then
                local md = vehicle:getModData()
                if md then
                    md.IKST_relocateKeysInvalid = true
                end
            end
        end
    end
