...
2. Log in: `gh auth login` (as the **repo owner**, with `repo` scope)
3. From repo root:
...
4. Enable **2FA** on the GitHub account (Settings → Password and authentication).
...
| PRs from others | **1 approving review** | Owner must review before merge |
| PRs from others | **Require CODEOWNERS** | `.github/CODEOWNERS` → repo owner |
| PRs from others | **Resolve conversations** | No drive-by merge with open threads |
| Owner workflow | **`enforce_admins=false`** | Owner can still push hotfixes directly to `master` |
...
1. GitHub → this repo → **Settings → Code security and analysis**
...
- Push to `master` (unless added as collaborator)
- Merge PRs without owner approval (when review rules apply)
...
| File | Role |
|------|------|
| `.github/CODEOWNERS` | PR review routing to the repo owner |
