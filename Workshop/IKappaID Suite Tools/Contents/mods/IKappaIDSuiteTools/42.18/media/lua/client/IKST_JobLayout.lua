...
function IKST_JobLayout.begin(panel, opts)
    ...
    local sideW = IKST_JobLayout.sidebarWidth(panel)
    panel.contentX = IKST_JobLayout.MARGIN + sideW
    panel.contentW = math.max(120, IKST_JobLayout.contentWidth(panel) - sideW)
    panel.logHeight = panel._logHeightOverride or IKST_JobLayout.LOG_HEIGHT
    panel.hintHeight = IKST_JobLayout.HINT_HEIGHT
    panel.jobHeaderY = layerTop
    panel.scrollTop = layerTop + IKST_JobLayout.HEADER_ROW
    panel.scrollHeight = panel.height - panel.scrollTop - panel.logHeight - panel.hintHeight - IKST_JobLayout.RESIZE_GRIP - 8
    ...
    if panel.jobScroll then
        panel.jobScroll:setX(sideW)
        panel.jobScroll:setY(IKST_JobLayout.toLayerY(panel, panel.scrollTop))
        panel.jobScroll:setWidth(math.max(120, panel.width - sideW - IKST_JobLayout.RESIZE_GRIP))
        panel.jobScroll:setHeight(panel.scrollHeight)
